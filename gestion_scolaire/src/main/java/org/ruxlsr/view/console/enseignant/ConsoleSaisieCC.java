...
public class ConsoleSaisieCC {
    ...
    public void executer() {
        ...
        for (Eleve el : eleves) {
            System.out.printf("Note CC pour %s : ", el.getNom());
            String input = scanner.nextLine();
            if (!input.isBlank()) {
                float note = Float.parseFloat(input);
                service.saisirNoteCC(el.getId(), cours.getId(), trimestre, note);
            }
        }

        System.out.println("Notes CC enregistrées.");
    }
}
